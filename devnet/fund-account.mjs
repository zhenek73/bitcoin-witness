 ...
 * "insufficient balance: <x> < 1000000000000000000" — it requires at least
 * 1 CTC on its own account before it will run. On a freshly created chain the
 * attestor account holds only dust, so it must be topped up from a genesis
...
if (!ADDRESS) {
  console.error('usage: node fund-account.mjs <address> [amountCTC] [endpoint]');
  process.exit(1);
}
...
await new Promise((resolve, reject) => {
  api.tx.balances.transferKeepAlive(ADDRESS, AMOUNT_CTC * 10n ** 18n)
